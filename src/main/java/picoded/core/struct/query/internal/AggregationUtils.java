...
public class AggregationUtils {
	...
	public static MutablePair<String[], String[]> extractAggregrationInfo(String[] aggregationArray) {
		...
		for (int i = 0; i < aggregationArray.length; ++i) {
			...
			if (leftBracket <= 0 || rightBracket != (funcNameAndField.length() - 1)) {
				throw new RuntimeException("Invalid aggregation function/parameter format : "
					+ funcNameAndField);
			}
	...
	public static MapReduceBase[] prepareMapReduceBaseArray(
		...
		for (int i = 0; i < len; ++i) {
			...
			if (base == null) {
				throw new RuntimeException("Unable to fetch Aggregation Function (MapReduceBase) for "
					+ funcNames[i]);
			}
	 ...
	 */
	public static BigDecimal[] computeMapReduceBase(MapReduceBase[] mapreduceArray,
		String[] fieldNames, Collection<Object> dataSet) {
