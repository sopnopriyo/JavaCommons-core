...
public class RequestHttpClient_GET_test {
	...
	public void basic_get_request() {
		...
		ResponseHttp responseHttp = requestHttpClient.get(mockWebServer.url("/").toString());
	...
	public void headers_get_request() throws InterruptedException {
		...
		// Retrieve mockResponse from server and assert the results
		ResponseHttp responseHttp = requestHttpClient.httpGet(mockWebServer.url("/").toString(),
			null, null, headers);
		assertEquals(responseHttp.statusCode(), 200);
		assertEquals(responseHttp.toString(), "hello, world!");
	...
	public void headers_responseHttp() {
		...
		mockWebServer.enqueue(new MockResponse().addHeader("first", "first value")
			.addHeader("first", "another value").addHeader("second", "single value"));
	...
	public void cookies_responseHttp() {
		...
		mockWebServer.enqueue(new MockResponse().addHeader("Set-Cookie", "cookie1=thiscookie")
	...
	public void basic_get_request_wrapper() throws InterruptedException {
		...
		mockWebServer.enqueue(new MockResponse().setBody("hello, world!"));
		// Retrieve mockResponse from server and assert the results
		ResponseHttp responseHttp = requestHttpClient.get(mockWebServer.url("/").toString(), null,
			null, null);
		assertEquals(responseHttp.statusCode(), 200);
		assertEquals(responseHttp.toString(), "hello, world!");
